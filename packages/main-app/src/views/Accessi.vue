...
<template>
  <div class="space-y-6">
    ...
    <PageHeader
      ...
      :description="`Gestisci gli accessi del sistema - Totale: ${accessi.length} accessi`"
    >
      <template #actions>
    ...
    <div class="card bg-base-100 shadow-sm">
      <div class="card-body max-md:p-3">
        <!-- Data Table Manager -->
        <DataTableManager
          :service="accessiService as unknown as FlexibleCrudService"
          :columns="tableColumns"
          entity-name="Accesso"
          entity-name-plural="Accessi"
          id-field="codice"
          list-route="/app/accessi"
          edit-route="/app/accessi"
          new-route="/app/accessi/new"
          :global-filter-fields="['codice', 'descrizione']"
          search-placeholder="Cerca per codice o descrizione..."
          export-filename="accessi-sistema"
          data-key="codice"
          filter-display="menu"
          scroll-height="600px"
          ...
          :delete-confirmation="{
            ...
            message: (accesso) => `Sei sicuro di voler eliminare l'accesso '${accesso.codice}'?`,
            warningText: 'Questa azione è irreversibile e potrebbe influenzare gli utenti che utilizzano questo accesso.'
          }"
...
const {
  data: accessi,
  loadData: loadAccessi
} = useCrudView<Accesso>(accessiService, {
  entityName: 'Accesso',
  entityNamePlural: 'Accessi',
  idField: 'codice',
  listRoute: '/app/accessi',
  editRoute: '/app/accessi',
  newRoute: '/app/accessi/new',
  deleteConfirmation: {
    title: 'Conferma eliminazione',
    message: (accesso) => `Sei sicuro di voler eliminare l'accesso \"${accesso.codice}\"?`,
    warningText: 'Questa azione è irreversibile e potrebbe influenzare gli utenti che utilizzano questo accesso.'
...
const onRowSelect = (event: any): void => {
  console.log('Accesso selezionato:', event.data)
}
